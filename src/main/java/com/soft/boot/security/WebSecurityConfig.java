...
/**
 * https://blog.csdn.net/Canon_in_D_Major/article/details/79688441?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.nonecase&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.nonecase
 */
@Configuration
@EnableWebSecurity
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    private void InControl(HttpSecurity http) throws Exception {
        http.authorizeRequests()       //配置安全策略
                ...
                .and().formLogin().loginPage("/login").usernameParameter("username").passwordParameter("password").permitAll().loginProcessingUrl("/user/login")
...
//                        session.setAttribute("username",((UserDetailsImpl)authentication.getPrincipal()).getUsername());
//                        session.setAttribute("role",((UserDetailsImpl)authentication.getPrincipal()).getRoles());
...
//                        out.close();
    ...
    private void UnControl(HttpSecurity http) throws Exception {
        http.csrf().disable()
                .authorizeRequests()
